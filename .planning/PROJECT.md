...
One person can define, launch, and evolve multiple kinds of agents from the same harness, including a coding agent backed by OpenCode, without rewriting the orchestration layer each time.
...
- [ ] User can define classic sub-agent profiles with a role, instructions, tools, and execution settings.
- [ ] User can persist and reuse classic sub-agent profiles across runs.
- [ ] User can define a coding sub-agent profile that targets OpenCode through ACP.
- [ ] User can launch the OpenCode-backed coding sub-agent from the harness.
- [ ] User can observe run status, output, and failure details for each launched sub-agent.
- [ ] The project keeps classic sub-agent configuration separate from coding-agent transport details.
...
- Finalized domain naming for "classic sub-agent" and "coding sub-agent" — terminology will be refined later.
...
- The current product goal is not to replace that runtime, but to add a higher-level harness that can configure and launch different kinds of agents.
- The first external coding backend to support is OpenCode, using ACP as the integration boundary.
- Terminology is intentionally provisional; the project currently uses "classic sub-agent" and "coding sub-agent" as working labels.
...
| Separate classic sub-agents from coding sub-agents | These are different configuration and execution problems | — Pending |
