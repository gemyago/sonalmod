...
- `NewRunParams` / `RunParamsBuilder.WithText` in [agent/runparams.go](./agent/runparams.go) — build `RunParams` with user ID, session ID, and fully qualified model (`provider/model-name`), then attach a single text user message for `Runner.Run` (model is required on each run; there is no runner-level default model option)
