...
- `RunnerArgs`: required `ProvidersConfigService` and required `AgentProfilesService`; profile-backed execution within `Runner.Run` uses the supplied profile service; session titles use the first provider model marked for summarization (see OpenAPI `ModelConfig.summarization`) with truncation fallback when none is designated (wired internally, not configurable on `RunnerArgs`)
