...
- `RunnerArgs`: required `ProvidersConfigService`; optional `AgentProfilesService` enables profile-backed execution within `Runner.Run`; session titles use the first provider model marked for summarization (see OpenAPI `ModelConfig.summarization`) with truncation fallback when none is designated (wired internally, not configurable on `RunnerArgs`)
