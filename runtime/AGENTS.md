...
- `RunnerArgs`: required `ProvidersConfigService`; session titles use the first provider model marked for summarization (see OpenAPI `ModelConfig.summarization`) with truncation fallback when none is designated (wired internally, not configurable on `RunnerArgs`)
- `NewRunParams` / `RunParamsBuilder.WithText` in [agent/runparams.go](./agent/runparams.go) — build `RunParams` with user ID, session ID, and fully qualified model (`provider/model-name`), then attach a single text user message for `Runner.Run` (the low-level runner still requires a model on each call; standard HTTP run endpoints instead require `profileName` and resolve execution through profile settings)
...
- HTTP API for agent execution [httpapi/](./httpapi/) (`NewHandler` and related types; `HandlerArgs.Runner` is [`agent.AgentRunner`](./agent/runner.go) — typically `*agent.Runner` — and `HandlerArgs.ProfileRunDispatcher` is typically built via [`agent.NewProfileRunDispatcher`](./agent/profile_run_dispatcher.go); standard `POST /agent-runs` and `POST /sessions/{sessionId}/agent-runs` requests require `profileName` and dispatch through profile execution settings, not request-level model overrides or public `/opencode-*` routes; `agent.ProvidersConfigService` and optional `agent.ModelsLister` (from `runner.ModelsLocator()`) enable provider and `GET /models` endpoints; the handler wraps the runner for background runs and unified ReadSession)
